...
if __name__ == "__main__":
    ...
    for folder in folders:
        ...
        for wall in wall_expand:
            new_dir = root_folder + '/new/' + folder + '_' + wall_name[data_aug - 1]
            ...
            for i in range(len(room_loc)):
                num_point = int(len(room_loc[i]) / 2)
                for j in range(-1, num_point-1):
                    ...
                    if minx == maxx and miny != maxy:
                        ...
                        if sum(sum(notation[miny:maxy+1, :minx])) == 0:
                            ...
                            # # 坐标去重
                            # if room_loc[i][2 * j + 2] == room_loc[i][2 * j + 4 - num_point] and \
                            #         room_loc[i][2 * j + 3] == room_loc[i][2 * j + 5 - num_point]:
                            #     for k in range(j+1, num_point - 1):
                            #         room_loc[i][2*k] = room_loc[i][k*2+2]
                            #         room_loc[i][2*k+1] = room_loc[i][2*k+3]
                            #     del room_loc[i][-2:]
